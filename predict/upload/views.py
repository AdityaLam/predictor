...
def auto_post(request):
    if request.method == 'POST':
        ...
        if p:
            ...
            html_content = "Based on the mutiple regression tests, we are %d percent certain that this part will come between %.2f and %.2f  days, with an average of %.2f days. <br>Based on %d data points. <br> Attached is a box plot of all the data we have on the part number specified.<p> Thanks,<br>Lam Part Predictor Team"  % (inter, conf[0][0], conf [1][0], summary.params[0], conf[0][1], conf[1],[1], summary.params[1], rpm+erf)
